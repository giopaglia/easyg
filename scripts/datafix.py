...
def split_data(data, r_train=0.6, r_val=0.3, r_test=0.1):
    ...
    return data_trainval, data_train, data_val, data_test
...
def main():
    ...
    trainval, train, val, test = split_data(filtered)

    save(trainval, 'data/trainval'+suffix+'.csv')
    save(train, 'data/train'+suffix+'.csv')
    save(val, 'data/validation'+suffix+'.csv')
    save(test, 'data/test'+suffix+'.csv')
